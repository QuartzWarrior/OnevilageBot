...
class YTDLSource(PCMVolumeTransformer):
    ...
    async def create_source(
        cls, ctx: commands.Context, search: str, *, loop: BaseEventLoop = None
    ):
...
class Music(commands.Cog):
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            voter.id not in ctx.voice_state.skip_votes
            and ctx.author.voice
            and ctx.author.voice.channel.id
            == ctx.guild.get_member(self.client.user.id).voice.channel.id
        ):
            ...
            if (
                total_votes
                >= (
                    len(ctx.guild.get_member(self.client.user.id).voice.channel.members)
                    - 1
                )
                / 100
                * 80
            ...
            await ctx.send(
                "You have already voted to skip this song OR you are not in the vc."
            )
