...
class CogFunctions(commands.Cog):
    ...
    async def disable(self, ctx, *, module):
        if (
            ctx.author.id == 713512807580303440
            or ctx.author.id == 802725022560944160
            or ctx.author.id == 1065714439380271164
        ):
            try:
                ...
                await ctx.send(
                    f"Module {module.strip()} is already disabled or doesn't exist"
                )
        else:
    ...
    async def reload(self, message, *, module):
        if (
            message.author.id == 713512807580303440
            or message.author.id == 802725022560944160
            or message.author.id == 1065714439380271164
        ):
            try:
